# -*- coding: utf-8 -*-
"""
Hybrid ML System — ensemble time-series forecasting.

Blends three forecasters of a stock price series (PETR4):
  * AR      — autoregressive linear baseline
  * RN      — MLP neural network (scikit-learn)
  * PSO+SVM — SVR whose hyper-parameters are tuned by particle-swarm optimization
and stacks them with linear regression, comparing mean/median/learned ensembles by MSE.

Requires an input file `petr4data.txt` (CSV with a `Close` column) in the working
directory. See README for usage and how to supply your own series.

Created 2019-06-19 by Rafael Aziz da Silva Rocha.
"""
...
def PSO_SVM(trainset, traintarget, testset, testtarget):
    ...
    # Melhor modelo: usa gbest (melhor solucao encontrada pelo enxame),
    # e nao pop[index] (posicao atual da particula, que ja se moveu)
    model = SVR(kernel='rbf', C=10**gbest[0], gamma=10**gbest[1], epsilon=10**gbest[2])
    model.fit(trainset, traintarget)
    predicts = model.predict(testset)
    return predicts, MSE(testtarget,predicts), gbest
...
plt.figure()
# AR
y_AR_t1 = np.sum(param_AR * np.array([serie.iloc[-1], 1]))
...
mse_AR_t = np.mean((Y_AR_t1.to_numpy() - testtarget.to_numpy())**2) 
plt.plot(testtarget, label='target')
plt.plot(testtarget.index,Y_AR_t1, label='AR')

# Neural Network
yy_t = scaler_RN.transform(np.array([serie.iloc[-1]]).reshape(1,-1))
...
mse_RN_t = np.mean((Y_RN_t1 - testtarget.to_numpy())**2)
...
model.fit(trainset, traintarget)
y_SVM_t1 = model.predict(np.array([serie.iloc[-1]]).reshape(1,-1))[0]
...
mse_SVM_t = np.mean((Y_SVM_t1 - testtarget.to_numpy())**2)
...
mse_RL_t = np.mean((Y_RL_t1 - testtarget.to_numpy())**2)
